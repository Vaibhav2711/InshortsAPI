...
def authenticate_user( payload: TokenData,db: Session = Depends(get_db)):
...
async def login_for_access_token(payload:TokenInput,db: Session = Depends(get_db)):
...
async def get_prediction(payload: VideoInputs,current_user: models.User = Depends(get_current_user),db: Session = Depends(get_db)):
    ...
    inputJson = {"actorId":actorId,"audioUrl":audioUrl}
